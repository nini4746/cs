...
- **[데일리 퀴즈](https://notes.nini4746.uk/quiz)** - 밀린 복습 노트들을 변형 문항으로 한 판에 처리. 통과한 노트는 `reviewed` 자동 기록. 복습 채널은 이것 하나로 일원화 (Anki 덱 생성은 중단).
- 사이트 빌드 때 자동 생성. 노트를 고치면 다음 배포에서 반영.

## 진도 추적

- 진실의 원천은 각 노트 frontmatter: `studied: YYYY-MM-DD` (첫 학습), `reviewed: [날짜, ...]` (복습 이력).
- 기록은 자기신고가 아니라 **퀴즈 통과로만** 됨: [비공개 사이트](https://notes.nini4746.uk)에서 노트를 공부한 뒤 우하단 **📝 퀴즈로 점검** 버튼 → 셀프체크 기반 **LLM 변형 문항**이 출제(같은 문제 반복 없음) → 답을 직접 타이핑 → AI 채점 평균 7/10 이상이면 `studied`(첫 통과) 또는 `reviewed`(재통과)가 자동 커밋되고 사이트 리빌드. 낙제하면 기록 없음, 다시 공부 후 재도전 (다음엔 다른 변형이 나옴). (`scripts/mark-studied.sh`는 관리용 백도어)
- 매일 자동 생성되는 페이지 (새벽 크론 빌드로 갱신):
  - [오늘의 레슨](today.md) - 다음 미학습 노트 자동 지정 + 밀린 복습 + 권장 학습법
  - [데일리 퀴즈](https://notes.nini4746.uk/quiz) - 복습 큐를 변형 문항으로 한 판에, 통과 시 `reviewed` 자동 기록
