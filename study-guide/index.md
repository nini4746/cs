...
- 기록은 자기신고가 아니라 **퀴즈 통과로만** 됨: [비공개 사이트](https://notes.nini4746.uk)에서 노트를 공부한 뒤 우하단 **📝 퀴즈로 점검** 버튼 → 셀프체크 질문에 답을 직접 타이핑 → AI 채점 평균 7/10 이상이면 `studied`(첫 통과) 또는 `reviewed`(재통과)가 자동 커밋되고 사이트 리빌드. 낙제하면 기록 없음, 다시 공부 후 재도전. (`scripts/mark-studied.sh`는 관리용 백도어)
