...
> [!question]- 분리 가능(separable) 상태와 얽힘(entangled) 상태를 어떻게 구분하는가?
> 결합 상태를 `|ψ⟩` = `|a⟩`⊗`|b⟩` 형태의 개별 큐빗 곱으로 쓸 수 있으면 분리 가능(얽힘 없음), 어떤 `|a⟩`, `|b⟩`로도 분해할 수 없으면 얽힘이다. 예로 (`|00⟩`+`|11⟩`)/√2는 분해 불가능해 얽힘이고, (`|00⟩`+`|01⟩`)/√2 = `|0⟩`⊗`|+⟩`는 분리 가능하다.

> [!question]- 벨 상태의 측정 상관이 초광속 통신을 가능하게 하지 않는 이유는?
> Φ⁺에서 두 큐빗의 측정 결과는 100% 상관되지만, 각 측정 결과 자체는 무작위(0/1이 각 50%)라 상대에게 보낼 정보를 실을 수 없다. no-signaling 정리에 따라 한쪽 측정 통계만으론 상대가 무엇을 했는지 감지할 수 없어, 광속 제한을 위반하지 않는다.

> [!question]- 벨 부등식 위반이 자연에 대해 말해주는 것은?
> CHSH 형태에서 국소 숨은 변수 이론은 |S| ≤ 2를 만족해야 하나, 양자역학은 S = 2√2 ≈ 2.83으로 이를 위반한다. 실험이 위반을 확인했으므로, 자연은 국소성(먼 사건이 즉시 영향 못 줌)과 실재론(측정 전 값이 정해짐)을 동시에 만족할 수 없다. 즉 국소적 실재론이 틀렸다.

> [!question]- 양자 순간이동이 no-cloning 정리와 모순되지 않는 이유는?
> 순간이동은 얽힘과 고전 2비트를 이용해 미지 상태를 전송하지만, 원본 큐빗의 상태는 측정 과정에서 파괴된다. 즉 상태가 복제되어 두 곳에 존재하는 것이 아니라 한 곳에서 다른 곳으로 이동하므로, 복제 불가 정리에 위배되지 않는다.

## 연습문제

> [!example]- 문제: `|+⟩`⊗`|0⟩` = (`|00⟩`+`|10⟩`)/√2에 CNOT(제어 q0, 대상 q1)을 적용한 결과를 계산하고 얽힘 여부를 판정하라.
> **풀이**
> CNOT은 제어가 `|1⟩`일 때만 대상을 뒤집는다.
> `|00⟩` → `|00⟩` (제어 0, 변화 없음).
> `|10⟩` → `|11⟩` (제어 1, 대상 뒤집힘).
> 결과 = (`|00⟩` + `|11⟩`)/√2 = Φ⁺ 벨 상태.
> `|a⟩`⊗`|b⟩`로 분해 불가능하므로 얽힘. H+CNOT이 얽힘을 생성하는 표준 회로임을 확인.

> [!example]- 문제: Φ⁺ = (`|00⟩`+`|11⟩`)/√2에서 q0을 측정해 1이 나왔다. q1을 측정하면 각 결과의 확률은?
> **풀이**
> q0 측정으로 `|11⟩` 항만 살아남아 측정 후 상태는 `|11⟩`으로 붕괴.
> 따라서 q1 측정 시 1이 나올 확률 = 100%, 0이 나올 확률 = 0%.
> 반대로 q0에서 0이 나왔다면 상태는 `|00⟩`으로 붕괴해 q1도 반드시 0. 이것이 완전 상관이며, 각 큐빗을 단독으로 보면 0/1이 각 50%로 무작위다.

## 파인만

> [!note]- 백지에 이 노트 핵심을 남에게 설명하듯 써보라. 막히면 그 부분만 다시.
> **점검 포인트**: (1) 임의의 2큐빗 상태가 주어졌을 때 얽힘인지 분리 가능인지 판정할 수 있는가. (2) 벨 부등식 |S|≤2와 양자 예측 2√2의 의미를 설명할 수 있는가. (3) 측정 상관이 있는데도 왜 정보 전달이 안 되는지(no-signaling) 말할 수 있는가.
