...
> [!question]- 큐빗 상태 `|ψ⟩` = α`|0⟩` + β`|1⟩`에서 α, β와 정규화 조건의 의미는?
> α, β는 복소수 확률 진폭으로, 그 자체는 관측 불가하고 제곱한 크기 |α|², |β|²가 측정 시 0, 1이 나올 확률로 드러난다. 정규화 조건 |α|²+|β|²=1은 전체 확률이 1임을 보장한다. 복소수라 위상을 가지며, 이 위상이 간섭의 원천이다.

> [!question]- 측정이 왜 비가역적이고, 이것이 알고리즘 설계에 주는 제약은?
> 측정하면 중첩이 하나의 기저 상태로 붕괴하고 원래 α, β 정보는 사라져 복원할 수 없다(no-cloning). 한 번의 측정으로 진폭을 읽어낼 수 없으므로, 양자 알고리즘은 측정 전에 정답의 진폭을 키우고 오답을 상쇄시켜 놓아야 한다.

> [!question]- 전역 위상과 상대 위상의 차이는?
> 전역 위상 e^(iγ)를 전체 상태에 곱해도 측정 확률이 불변이라 물리적으로 무의미하다. 반면 상대 위상 φ(`|1⟩` 성분에 붙는 e^(iφ))는 간섭에서 결정적 역할을 해 의미가 있다. 그래서 단일 큐빗의 자유도는 4실수가 아니라 θ, φ 두 실수로, 블로흐 구 표면의 한 점에 대응한다.

> [!question]- "중첩 = 동시에 0이면서 1"이라는 표현이 왜 부정확한가?
> 중첩은 물리적으로 두 값을 동시에 갖는 것이 아니라, 측정 전까지 확률 진폭 α, β로만 존재하는 상태다. `|+⟩`를 측정하면 0과 1이 각각 50%로 나올 뿐이다. "동시에"라는 표현보다 "측정 전엔 진폭으로만 규정된다"가 정확하다.

## 연습문제

> [!example]- 문제: 상태 `|ψ⟩` = (1/2)`|0⟩` + (√3/2)`|1⟩`에서 측정 결과 각각의 확률과 정규화 여부를 확인하라.
> **풀이**
> α = 1/2 → |α|² = 1/4. β = √3/2 → |β|² = 3/4.
> P(0) = 1/4 = 25%, P(1) = 3/4 = 75%.
> 정규화: |α|²+|β|² = 1/4 + 3/4 = 1 → 유효한 큐빗 상태.
> 측정하면 25%로 `|0⟩`, 75%로 `|1⟩`로 붕괴한다.

> [!example]- 문제: `|+⟩` = (`|0⟩`+`|1⟩`)/√2와 `|−⟩` = (`|0⟩`−`|1⟩`)/√2를 계산 기저에서 측정하면 결과가 같은가? 무엇이 이들을 구분하는가?
> **풀이**
> `|+⟩`: |1/√2|² = 1/2씩 → P(0)=P(1)=50%.
> `|−⟩`: |1/√2|²와 |−1/√2|² = 1/2씩 → P(0)=P(1)=50%.
> 계산 기저 측정만으론 두 상태를 구분 불가(확률 동일). 차이는 상대 위상(π)뿐이며, 이는 X 기저(`|+⟩`,`|−⟩`)로 측정하거나 H를 걸어 계산 기저로 돌려야 드러난다. H`|+⟩`=`|0⟩`, H`|−⟩`=`|1⟩`으로 확정 구분된다.

## 파인만

> [!note]- 백지에 이 노트 핵심을 남에게 설명하듯 써보라. 막히면 그 부분만 다시.
> **점검 포인트**: (1) 진폭 α, β와 확률 |α|²·|β|², 정규화 조건의 관계를 설명할 수 있는가. (2) 측정 붕괴가 왜 비가역이고 알고리즘 설계를 제약하는지 말할 수 있는가. (3) 전역 위상은 무의미하고 상대 위상은 의미 있는 이유를 간섭과 연결해 설명할 수 있는가.
