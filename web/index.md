...
- [x] [[critical-rendering-path]] - HTML 파싱 → DOM/CSSOM → 레이아웃 → 페인트 → 컴포지팅
- [x] [[javascript-event-loop]] - 이벤트 루프, 태스크/마이크로태스크 큐, async가 도는 원리
- [x] [[reflow-repaint]] - 리플로우 유발 요인, transform이 싼 이유 (컴포지터 전용)

## 2. 네트워크 접점

- [x] [[http-for-web]] - 캐싱 헤더 전략 (ETag, Cache-Control), 압축, 우선순위 → network/http 기반
- [x] [[cors]] - same-origin policy, preflight, CORS 오류의 정확한 원인
- [x] [[websockets-sse]] - 실시간 통신 선택지, 폴링 vs SSE vs WebSocket
...
- [x] [[rendering-strategies]] - CSR/SSR/SSG/ISR, hydration 비용
