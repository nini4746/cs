...
> [!question]- 암호화가 변조를 못 막는 구체적 예는?
> CTR/스트림 암호는 c=m⊕keystream이라 공격자가 암호문 비트를 뒤집으면 평문의 같은 위치 비트가 뒤집힌다(키를 몰라도). "$100"의 특정 비트를 조작해 "$900"으로 바꿀 수 있다.

> [!question]- HMAC의 이중 해시 구조가 방어하는 공격은?
> 단순 H(k‖m)은 Merkle-Damgård 길이 확장으로 위조 가능하다. HMAC(k,m)=H((k⊕opad)‖H((k⊕ipad)‖m))의 이중 해시가 내부 상태 이어받기를 막아 이를 차단한다.

> [!question]- encrypt-then-MAC이 증명상 안전한 이유는?
> 태그를 암호문에 대해 계산하고 먼저 검증하므로, 유효하지 않은 암호문은 복호조차 되지 않는다. 따라서 패딩 오라클 같은 복호 기반 오라클 공격이 원천 차단된다.

> [!question]- MAC과 서명의 차이(부인방지)는?
> MAC은 대칭(공유 키)이라 양쪽 모두 태그를 만들 수 있어 부인방지가 없다. 서명은 개인키로만 서명하고 공개키로 검증하므로 서명자를 특정할 수 있고 부인방지를 제공한다.

## 연습문제

> [!example]- 문제: 시스템이 MAC-then-Encrypt(평문에 MAC 후 CBC 암호화)를 쓴다. 왜 패딩 오라클에 노출되는지, encrypt-then-MAC이 어떻게 막는지 진단하라.
> **풀이**
> MAC-then-Encrypt에선 수신자가 먼저 복호·패딩 검사를 하고 그 다음 MAC을 검증한다.
> 패딩 유효/무효 응답 차이가 오라클이 되어 공격자가 CBC 암호문을 바이트 단위로 복원할 수 있다.
> Encrypt-then-MAC은 MAC부터 검증하고 실패 시 복호 자체를 하지 않으므로 패딩 검사 오라클이 생기지 않는다.

> [!example]- 문제: JWT가 HS256(HMAC-SHA256)으로 서명된다. 서버가 토큰의 alg 헤더를 신뢰해 검증기를 고르면 어떤 공격이 가능하고 어떻게 막는지 분석하라.
> **풀이**
> alg=none 공격: 공격자가 alg를 none으로 바꾸고 서명을 제거하면, 서버가 검증을 생략해 임의 토큰을 위조할 수 있다.
> RS256→HS256 혼동: 공격자가 alg를 HS256으로 바꾸고 서버의 RSA 공개키를 HMAC 키로 쓰게 유도해 유효 태그를 위조한다.
> 완화: 서버가 alg를 코드에 고정하고 화이트리스트로 강제하며, 토큰의 alg 헤더를 신뢰하지 않는다.

## 파인만

> [!note]- 백지에 왜 기밀성만으론 부족하고 MAC·AEAD·encrypt-then-MAC이 각각 무엇을 하는지 설명하듯 써보라. 막히면 그 부분만 다시.
> **점검 포인트**: (1) 변조 예(비트 뒤집기)와 기밀성 ≠ 무결성, (2) HMAC과 길이 확장 방어, (3) 조합 순서와 오라클 공격.
