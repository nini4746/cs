...
> [!question]- Git의 네 가지 객체는?
> blob(파일 내용), tree(디렉토리, 파일명→blob/tree 매핑), commit(스냅샷=tree+부모+메타), tag(주석 태그). 모두 내용의 해시(SHA)로 저장된다.

> [!question]- 내용 주소(content-addressable) 저장이 주는 두 가지 이점은?
> 같은 내용은 같은 해시 → 한 번만 저장(중복 제거). 내용이 바뀌면 해시가 바뀌므로 변조를 감지(무결성).

> [!question]- Git에서 브랜치 생성이 왜 싼가?
> 브랜치는 커밋을 가리키는 이름(ref)일 뿐이다. 생성은 커밋 해시를 담은 파일 하나 만들기(O(1))이고, 커밋은 불변이라 포인터만 움직인다.
...
> [!question]- 커밋은 diff인가 스냅샷인가?
> 스냅샷이다. 각 커밋은 그 시점 전체 트리를 가리키고, 부모 링크로 이전 커밋과 연결되어 DAG를 이룬다(변경분이 아니라 상태).

## 연습문제

> [!example]- 문제: 팀원이 `git reset --hard HEAD~3`으로 커밋 3개를 날렸다며 당황한다. 복구 절차를 명령으로 제시하라.
> **풀이**
> 1. `git reflog` - HEAD가 거쳐온 위치 목록에서 reset 직전 커밋 해시를 찾는다(예: `abc123 HEAD@{1}`).
> 2. `git reset --hard abc123` 또는 안전하게 `git branch recover abc123`로 새 브랜치에 복원.
> 커밋 객체는 GC 전까지 남아 있으므로 reflog로 되찾을 수 있다.

> [!example]- 문제: `.git/refs/heads/main` 파일에는 무엇이 들어 있고, "브랜치는 포인터"라는 말과 어떻게 연결되는지 설명하라.
> **풀이**
> 그 파일에는 커밋 해시(SHA) 한 줄이 들어 있다. 즉 브랜치 = 특정 커밋을 가리키는 해시를 담은 파일이다.
> 커밋하면 이 파일의 해시가 새 커밋으로 갱신될 뿐이고 커밋 객체 자체는 불변이다. HEAD는 보통 이 브랜치 ref를 가리켜 "현재 위치"를 나타낸다. 그래서 브랜치 생성·이동이 파일 하나 조작이라 싸다.

## 파인만

> [!note]- 백지에 blob/tree/commit/ref로 Git 저장 모델을 남에게 설명하듯 그려보라. 막히면 그 부분만 다시.
> **점검 포인트**: (1) 네 객체와 내용 주소(해시) 저장, (2) 브랜치=포인터, HEAD, ref가 전부라는 것, (3) 커밋 불변성과 reflog 안전망.
