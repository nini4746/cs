...
> [!question]- merge와 rebase의 차이는?
> merge는 머지 커밋(부모 2개)을 만들어 분기·병합 히스토리를 그대로 보존(비선형, 안전). rebase는 내 커밋들을 다른 브랜치 끝으로 재적용해 선형 히스토리를 만들지만 새 커밋을 생성(해시 바뀜, 히스토리 재작성).

> [!question]- rebase의 황금 규칙은?
> 공유된(push된) 브랜치는 rebase하지 마라. 히스토리를 재작성해 남의 커밋 기반을 깨뜨린다. rebase는 로컬 커밋 정리·최신화에만 쓴다.

> [!question]- trunk-based development가 통합 지옥을 피하는 원리는?
> main에 하루에도 여러 번 자주 통합하고 브랜치 수명을 짧게(몇 시간~하루) 유지한다. 자주 통합할수록 충돌이 작아지고, 미완성 기능은 feature flag로 숨긴다. CI/CD와 잘 맞는다.

> [!question]- "작고 자주" 통합이 "크고 드문" 것보다 나은 이유는?
> 큰 브랜치를 오래 두면 main과 멀어져 충돌이 폭발(merge hell)한다. 작게 자주 통합하면 충돌이 작고 리뷰가 쉽다.

> [!question]- 좋은 커밋의 조건은?
> 원자적(하나의 논리적 변경, 기능과 리팩토링 분리), 명확한 메시지(무엇을·왜, 제목+본문), 컨벤셔널 커밋 형식(feat:/fix: 등으로 자동 버저닝·changelog).

## 연습문제

> [!example]- 문제: 팀원이 이미 push해 다른 사람들이 받아간 공유 브랜치를 rebase 후 force-push했다. 무슨 문제가 생기고 어떻게 대응하나?
> **풀이**
> rebase는 새 커밋(다른 해시)을 만들어 히스토리를 재작성하므로, 이미 그 브랜치를 받아간 사람들의 로컬 히스토리와 갈라진다. 그들이 pull하면 충돌·중복 커밋이 생긴다.
> 대응: reflog로 rebase 이전 상태를 복원해 원래 히스토리로 되돌린 뒤 팀에 공유. 이후엔 공유 브랜치는 merge만, rebase는 로컬 전용이라는 황금 규칙을 지킨다.

> [!example]- 문제: 소규모 팀의 GitHub flow 협업 규칙(브랜치·PR·머지·배포)을 5단계로 정리하라.
> **풀이**
> 1. main에서 짧은 수명의 feature 브랜치를 딴다.
> 2. 작게 커밋(원자적, 컨벤셔널 메시지)하고 자주 푸시.
> 3. PR을 열어 코드 리뷰(버그·설계·가독성)를 받는다.
> 4. CI 게이트(테스트·린트) 통과가 머지 조건.
> 5. main에 머지하면 자동 배포. 브랜치를 짧게 유지해 통합 지옥을 피한다.

## 파인만

> [!note]- 백지에 merge/rebase와 trunk-based 흐름을 남에게 설명하듯 그려보라. 막히면 그 부분만 다시.
> **점검 포인트**: (1) merge(보존·안전) vs rebase(선형·재작성)와 황금 규칙, (2) 작고 자주 통합이 충돌을 줄이는 원리, (3) PR·CI 게이트·좋은 커밋이 협업 기록이 되는 이유.
