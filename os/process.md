...
> [!question]- 프로그램과 프로세스의 차이는 무엇인가?
> 프로그램은 디스크에 있는 정적 파일(코드 + 데이터)이고, 프로세스는 그것을 실행하며 생긴 동적 실체다. 프로세스는 주소 공간, 레지스터 상태(PC/SP), 열린 파일, 커널 상태(PCB)를 갖는다. 하나의 프로그램에서 여러 프로세스가 생길 수 있다(같은 프로그램을 여러 번 실행).

> [!question]- 왜 유닉스는 프로세스 생성을 fork와 exec 두 단계로 나눴는가?
> fork와 exec 사이에서 자식의 환경을 조정할 수 있기 때문이다. 파일 디스크립터 리다이렉트(`>`), 파이프 연결(`|`) 같은 설정을 자식이 새 프로그램으로 변신하기 전에 해둘 수 있다. 이 분리가 셸의 조합성(redirection, pipe)을 만든다.

> [!question]- 좀비와 고아 프로세스의 차이는? 어느 쪽이 문제가 되나?
> 좀비는 자식이 죽었는데 부모가 wait을 안 해서 종료 상태를 PCB에 남긴 채 잔존하는 경우로, PID를 점유한다. 고아는 부모가 먼저 죽어 init(PID 1)이 입양해 대신 수확한다. 고아는 init이 자동 수확하므로 문제가 없고, 좀비는 부모가 wait을 계속 안 하면 누적되어 PID 고갈을 일으킨다.

> [!question]- Blocked 상태는 왜 타이머 인터럽트로 깨어나지 않는가?
> Blocked는 I/O 등 자원/이벤트를 기다리는 상태라 CPU를 줘도 진행할 수 없다. 그래서 타임슬라이스 만료(타이머 인터럽트)가 아니라, 기다리던 자원이 준비됐다는 I/O 완료 인터럽트로 깨어나 Ready로 간다. 반면 Running↔Ready 전환이 스케줄러의 몫이다.

## 연습문제

> [!example]- 문제: 다음 코드에서 부모와 자식이 각각 무엇을 출력하는지 추적하라. `pid_t p = fork(); printf("%d\n", p);` 이후 `getpid()`로 자기 PID를 찍는다면?
> **풀이**
> fork는 한 번 호출되어 두 번 반환한다. 부모에게는 자식의 PID(예: 32762)를, 자식에게는 0을 반환한다. 따라서 부모는 `32762`를, 자식은 `0`을 출력한다.
> `getpid()`는 각자 자기 PID를 돌려주므로 부모는 자기 PID(32761), 자식은 자기 PID(32762)를 찍는다. 즉 자식은 "fork 반환값 0, 내 pid 32762, 부모 32761"이 된다. 복제 직후 둘은 독립이라 이후 변수 변경은 서로 영향 없다(copy-on-write).

> [!example]- 문제: 셸이 `ls > out.txt`를 실행하는 흐름을 시스템 콜 단위로 써라.
> **풀이**
> 1. `fork()` - 셸이 자기 복제본(자식)을 만든다.
> 2. 자식에서 stdout(fd 1)을 `out.txt`로 리다이렉트한다(예: `open("out.txt")` 후 `dup2`로 fd 1에 덮어쓰기). fork와 exec 사이의 이 조정이 핵심이다.
> 3. 자식이 `exec("ls")` - 주소 공간이 ls로 통째 교체되지만 PID와 리다이렉트한 fd 테이블은 유지된다. 그래서 ls의 출력이 out.txt로 간다.
> 4. 부모(셸)는 `wait()`로 자식 종료를 기다리고 종료 상태를 수확한다(좀비 방지).

> [!example]- 문제: 어떤 서버가 요청마다 fork로 워커를 만들고 처리 후 워커는 exit하는데, 시간이 지나자 "fork: cannot allocate" 에러가 난다. 원인과 수정은?
> **풀이**
> 원인: 부모(서버)가 자식에 대해 wait/waitpid를 호출하지 않아 종료한 워커들이 좀비로 쌓인다. 좀비는 리소스는 거의 없지만 PID를 점유하므로 계속 누적되면 PID가 고갈되어 새 fork가 실패한다.
> 수정: 자식 종료를 수확한다. `SIGCHLD` 핸들러에서 논블로킹 `waitpid(-1, ..., WNOHANG)`을 루프로 돌려 죽은 자식을 모두 거두거나, `SIGCHLD`를 `SIG_IGN`으로 설정해 커널이 자동 수확하게 한다.

## 파인만

> [!note]- 백지에 이 노트 핵심을 남에게 설명하듯 써보라. 막히면 그 부분만 다시.
> **점검 포인트**: 이해했다면 답할 수 있어야 하는 핵심 3가지를 짧게.
> 1. 프로세스 = 프로그램 + 실행 상태, 그리고 그 상태가 PCB(주소 공간·레지스터·열린 파일·커널 상태)에 담긴다는 것.
> 2. fork(2번 반환) + exec(주소 공간 교체) + wait(수확)의 역할과, 왜 fork/exec를 나눴는지.
> 3. 프로세스 상태 전이(Running/Ready/Blocked)와 좀비·고아가 생기는 조건.
