...
> [!question]- 렉시컬 스코프와 동적 스코프의 차이, 그리고 렉시컬이 표준인 이유는?
> 렉시컬은 코드가 "정의된 위치"로, 동적은 "호출 시점의 콜 스택"으로 변수를 결정한다. 렉시컬은 코드만 보면 어떤 변수를 참조할지 알 수 있어(호출 경로 몰라도) 추론과 최적화가 쉬워 표준이 됐다.

> [!question]- 클로저는 "값"을 캡처하나 "변수"를 캡처하나? 그 결과 생기는 함정은?
> 대부분 언어는 변수(참조)를 캡처한다. 그래서 캡처 후 그 변수가 바뀌면 클로저도 바뀐 값을 본다. 루프에서 만든 여러 클로저가 같은 변수를 공유해 전부 마지막 값을 보는 late binding 버그가 대표적이다.

> [!question]- 바깥 함수가 끝났는데도 클로저가 변수에 접근할 수 있는 이유는?
> 캡처된 변수(upvalue)는 스택 프레임이 사라져도 살아야 하므로 힙으로 옮겨진다(closed upvalue). 그래서 make_counter가 반환된 뒤에도 inc가 count에 접근할 수 있다.

> [!question]- 왜 "함수가 일급 값"이면 클로저가 필연인가?
> 함수를 반환·전달할 수 있으면 그 함수가 참조하는 바깥 변수도 반환 이후까지 살아있어야 한다. 그 변수를 붙잡아 유지하는 것이 바로 클로저다.

## 연습문제

> [!example]- 문제: 아래 코드의 출력을 예측하고, `[0,1,2]`가 나오도록 고쳐라.
> 
> ```python
> funcs = [lambda: i for i in range(3)]
...
> **풀이**
> 
> 출력은 `[2, 2, 2]`. 세 람다가 값이 아니라 같은 변수 `i`를 캡처하고, 호출 시점엔 루프가 끝나 `i=2`이므로 전부 2를 본다(late binding). 각 반복의 값을 즉시 붙잡으려면 기본 인자로 값 캡처:
> 
> ```python
...
> ```
> 
> 기본 인자는 정의 시점에 평가돼 그 순간의 `i`를 값으로 고정한다.

> [!example]- 문제: `c1 = make_counter()`를 세 번, `c2 = make_counter()`를 한 번 호출한 결과를 추적하고, 왜 c1과 c2가 독립인지 설명하라.
> 
> **풀이**
> 
> `c1() c1() c1()` → 1, 2, 3. `c2()` → 1. make_counter를 호출할 때마다 새 스택 프레임과 새 `count` 바인딩이 생기고, 반환된 inc가 그 특정 count를 캡처(힙으로 이동)한다. c1과 c2는 서로 다른 count upvalue를 붙잡으므로 상태가 완전히 분리된다.

## 파인만

> [!note]- make_counter 예제를 백지에 그리고, 함수가 반환된 뒤 count가 어디에(스택? 힙?) 살아남는지, 두 카운터가 왜 독립인지 남에게 설명하듯 써보라. 이어서 [2,2,2] 버그를 같은 그림으로 설명하라.
> **점검 포인트**: 이해했다면 답할 수 있어야 하는 핵심 3가지 - (1) 렉시컬 스코프가 참조 대상을 어떻게 결정하나, (2) 값 캡처 vs 변수 캡처, (3) upvalue가 힙으로 옮겨지는 이유.
