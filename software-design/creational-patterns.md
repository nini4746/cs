...
> [!question]- 팩토리 패턴이 어떻게 의존성 역전(DIP)을 돕나?
> 사용 코드가 `new MySQLDatabase()` 같은 구체 클래스 대신 팩토리가 반환하는 인터페이스(Database)에만 의존하게 된다. 무엇을 만들지는 팩토리가 결정하므로 구현을 자유롭게 교체할 수 있다.

> [!question]- 빌더는 telescoping constructor의 무엇을 해결하나?
> 인자가 많은 생성자는 `new Pizza(true,false,true,"large")`처럼 뭐가 뭔지 안 보이고 선택적 인자 조합이 폭발한다. 빌더는 메서드 이름으로 의미를 드러내고 필요한 것만 설정해 가독성과 불변 객체 조립을 준다.

> [!question]- 싱글턴이 안티패턴 취급받는 이유 세 가지는?
> (1) 전역 상태라 어디서든 접근돼 숨은 의존성·결합을 만든다, (2) 목으로 대체가 안 돼 테스트가 어렵다, (3) 공유 가변 상태라 동시성 위험이 있다. 대개 DI로 하나의 인스턴스를 주입하는 게 낫다.

## 연습문제

> [!example]- 문제: 서비스 코드 곳곳에 `db = new MySQLDatabase()`가 박혀 있어 테스트용 DB나 Postgres로 못 바꾼다. 개선하라.
> **풀이**
> 구체 클래스 직접 생성 → 구체 결합(DIP 위반).
> `Database` 인터페이스를 두고 팩토리(`DatabaseFactory.create(config)`)나 DI로 구현을 주입한다. 서비스는 인터페이스에만 의존하므로 테스트에선 인메모리 페이크, 운영에선 MySQL을 꽂을 수 있다.

> [!example]- 문제: `Logger.getInstance()` 싱글턴을 여러 클래스가 직접 부른다. 단위 테스트에서 로그를 검증하거나 끄기가 어렵다. 리팩터링하라.
> **풀이**
> 전역 싱글턴이 숨은 의존성이라 테스트에서 대체 불가.
> `Logger` 인터페이스를 만들고 필요한 클래스가 생성자로 주입받게 한다(DI). 운영에선 컨테이너가 단일 인스턴스를 관리(scope=singleton)하고, 테스트에선 목/페이크 로거를 주입한다. 전역이 아니라 명시적 의존이 된다.

## 파인만

> [!note]- 백지에 "왜 그냥 new를 안 쓰고 생성 패턴을 쓰나"를 예시로 설명하고, 반대로 언제 그냥 new가 나은지도 써보라.
> **점검 포인트**: (1) 팩토리·빌더가 각각 해결하는 문제(결합/가독성), (2) 싱글턴의 위험과 DI 대안, (3) 패턴이 복잡도를 줄일 때만 정당하다는 과용 경계.
